...
namespace neutron{
std::tuple<float,float> test(debug::LDG &model,Dataset &dataset){
    model.eval();
...
void window_slide_train(){
    ...
    auto model = debug::LDG(NUM_NODES,
                        HIDDEN_DIM);
...
//        std::cout<<"model.parameters():"<<model.parameters()<<std::endl;
  ...
  std::cout<<"run time: "<<cpuSecond()-run_start<<std::endl;
