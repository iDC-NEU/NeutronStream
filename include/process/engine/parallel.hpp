...
namespace neutron{
   template <typename DerviedSubGraph,bool IsCacheSubg=false,size_t CutNum=50>  ///111111
   class ParallelEngine: public Engine{
    public:
      typedef std::shared_ptr<ParallelEngine> ptr;
      static ParallelEngine::ptr ParallelEngineptr;
      static ParallelEngine::ptr GetInstanceThreadSafe();
  ...
  //                numEdges[Id_index[std::this_thread::get_id()]]+=esgList[cur_id].num_edges();
   ...
   typename ParallelEngine<DerviedSubGraph,IsCacheSubg,CutNum>::ptr 
   ParallelEngine<DerviedSubGraph,IsCacheSubg,CutNum>::ParallelEngineptr=nullptr;
   
   template <typename DerviedSubGraph,bool IsCacheSubg,size_t CutNum>
   typename ParallelEngine<DerviedSubGraph,IsCacheSubg,CutNum>::ptr 
   ParallelEngine<DerviedSubGraph,IsCacheSubg,CutNum>::GetInstanceThreadSafe(){
    static std::mutex mtx;
    if(ParallelEngineptr==nullptr){
      std::unique_lock<std::mutex> locker(mtx);
      if(ParallelEngineptr==nullptr){
        ParallelEngineptr.reset(new (std::nothrow) ParallelEngine);
      }
    }
    return ParallelEngineptr;
   } 
