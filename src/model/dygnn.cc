...
namespace neutron
{
  ...
  float drop) {
	...
	// NEUTRON_LOG_DEBUG(LOG_ROOT()) << "DyGNN contruster and it's device "<< device << std::endl;
    decayer = new Decayer();
    decayer->to(device);
  ...
  std::tuple<torch::Tensor,float> DyGNN::forward(std::vector<Event> &events) {
    ...
    std::function<torch::Tensor(DyGNNSubG &)> time_encoder = [&](DyGNNSubG &dySubGraph) -> torch::Tensor {
        ...
        auto head_delta_t = torch::tensor(time_delta_hour(cur_time, src_pre_time)).to(device);
        auto tail_delta_t = torch::tensor(time_delta_hour(cur_time, dst_pre_time)).to(device);
...
//4 - node_representation
    ...
    run<DyGNNSubG>(events,update_graph,update_fun,sampler);
