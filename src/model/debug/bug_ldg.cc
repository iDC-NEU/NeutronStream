...
namespace neutron{
    ...
    std::tuple<torch::Tensor, torch::Tensor> LDG::forward(std::vector<Event> &events)
    {
        ...
        for(size_t i=0;i<events.size();i++){
            ...
            DySubGraph::ptr subg(new LDGSubg(events[i]));
            Session::GetInstance()->addTimer("update_graph",cpuSecond()-t_update_graph);
            double t_update_emb=cpuSecond();


            update_fun(*(dynamic_cast<LDGSubg*>(subg.get())));
            Session::GetInstance()->addTimer("update_emb",cpuSecond()-t_update_emb);
            double sample_time=cpuSecond();

            sampler->sample(subg);
            Session::GetInstance()->addTimer("sample",cpuSecond()-sample_time);
